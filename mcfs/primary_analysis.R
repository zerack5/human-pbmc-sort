...
# 假设您有一个指定的基因列表 selected_genes_list


#########制作筛选标记基因列表
selected_genes_list <- c("GeneA", "GeneB", "GeneC")
# 创建逻辑向量，标识哪些基因在指定的基因列表中
data$selectedgene <- ifelse(data$selectedgene %in% selected_genes_list, data$selectedgene, NA)
...
ggsave("PBMC26_m_leftcontrol_rightas.tiff",device = "tiff",width =10,height =10,dpi = 300,path = "D:/data_anlysis/bulk_control_AS/bulk26/figure")
...
write.csv(result2,file = "Cell486_casecontrol_cibersortresult.csv",row.names = T)
...
ggplot(res,aes(cell.type,value,fill = group)) + 
  ...
  #使用 stat_compare_means() 函数在图表上添加统计学检验的结果，其中 group 是分组变量，使用 "kruskal.test" http://127.0.0.1:29679/graphics/plot_zoom_png?width=1920&height=1017方法计算组间差异。检验结果使用显著性水平标注在图表上。
  
  ########将表格当中的用药前中后三种组别的差异分别做三次检验来体现出来
  #geom_signif(comparisons = list(c("control", "as")), test = "wilcox.test",
...
ggsave("PBMC26_m_486_cibersort_all.tiff",device = "tiff",width =18,height =9,dpi = 300,path = "D:/data_anlysis/bulk_control_AS/bulk26/cibersort/figure")
...
ggsave("PBMC26_m_486_cibersort.tiff",device = "tiff",width =10,height =7,dpi = 300,path = "D:/data_anlysis/bulk_control_AS/bulk26/cibersort/figure")
...
# 转换为小提琴图
ggplot(filtered_res, aes(cell.type, value, fill = group)) + 
  geom_violin() +  # 使用 geom_violin() 替换 geom_boxplot() 来创建小提琴图
  theme_bw() + 
  labs(x = "Cell Type", y = "Estimated Proportion") +
  theme(legend.position = "top") + 
  theme(axis.text.x = element_text(angle = 80, vjust = 0.5, size = 14, face = "italic", color = 'black'),
        axis.text.y = element_text(face = "italic", size = 14, color = 'black')) +
  scale_fill_manual(values = c("control" = "green", "as" = "red")) +
  stat_compare_means(aes(group = group), label = "p.format", size = 3, method = "kruskal.test") +
  geom_signif(comparisons = list(c("control", "as")), y_position = c(0.9, 0.8), annotations = c(",", "**"), tip_length = 0.02)
...
ggplot(res,aes(cell.type,value,fill = group)) + 
  ...
  #scale_fill_nejm()+
  #scale_fill_nejm()是ggplot2中一个用于调整填充颜色的函数，它用于设置图形中填充颜色的默认值，使其与New England Journal of Medicine（NEJM）杂志的风格一致。
  theme(text = element_text(family = "sans", # 设置字体族为sans-serif
                            face = "bold", # 设置字体样式为粗体
                            size = 14
  ...
  stat_compare_means(aes(group = group),label = "p.format",size=5,method = "kruskal.test")

ggsave("PBMC26_m_lm22_sig_cibersort_all.tiff",device = "tiff",width =24,height =9,dpi = 300,path = "D:/data_anlysis/bulk_control_AS/bulk26/cibersort/figure")
...
ggsave("PBMC26_m_lm22_sig_cibersort.tiff",device = "tiff",width =7,height =7,dpi = 300,path = "D:/data_anlysis/bulk_control_AS/bulk26/cibersort/figure")
