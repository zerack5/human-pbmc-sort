heatdata<-read.csv("C:/Users/a/Desktop/as分选课题/分选课题/1/CD8_res.csv", header = TRUE)
colnames(heatdata)
heatdata <- heatdata[,c("X","log2FoldChange","pvalue","valpadj" )]
...
ggplot(heatdata,aes(log2FoldChange,-log10(padj)))+ 
  ...
  labs(x=expression(Log[2]*" Fold Change"),
       y=expression(-Log[10]*" (pvalue)")) #修改坐标轴命名细节


data <- heatdata
data$changed <- factor(ifelse(data$pvalue < 0.000001 & abs(data$log2FoldChange) > 3, 
...
data$selectedgene <- ifelse(data$pvalue<0.0000001 & abs(data$log2FoldChange)>3,data$X,NA)
head(data)
