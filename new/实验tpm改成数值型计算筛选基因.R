...
# 找出数据框中名为"label"的列的索引
label_column_index <- which(names(new_CD4data) == "label")
# 使用lapply()函数来转换除"label"列以外的所有列为数值型
new_CD4data[, -label_column_index] <- lapply(new_CD4data[, -label_column_index], as.numeric)
...
# 找出数据框中名为"label"的列的索引
label_column_index <- which(names(new_CD8data) == "label")
# 使用lapply()函数来转换除"label"列以外的所有列为数值型
new_CD8data[, -label_column_index] <- lapply(new_CD8data[, -label_column_index], as.numeric)
...
# 找出数据框中名为"label"的列的索引
label_column_index <- which(names(new_modata) == "label")
# 使用lapply()函数来转换除"label"列以外的所有列为数值型
new_modata[, -label_column_index] <- lapply(new_modata[, -label_column_index], as.numeric)
...
# 找出数据框中名为"label"的列的索引
label_column_index <- which(names(new_nkdata) == "label")
# 使用lapply()函数来转换除"label"列以外的所有列为数值型
new_nkdata[, -label_column_index] <- lapply(new_nkdata[, -label_column_index], as.numeric)



########查找可能存在的缺失值，缺失值会在绘图里面出问题
na_matrix <- is.na(new_CD4data)
print(na_matrix)
# 查找包含NA值的行
rows_with_na <- which(rowSums(na_matrix) > 0)
# 查找包含NA值的列
cols_with_na <- which(colSums(na_matrix) > 0)
# 输出包含NA值的行和列的索引
print(paste("Rows with NA values:", rows_with_na))
print(paste("Columns with NA values:", cols_with_na))
