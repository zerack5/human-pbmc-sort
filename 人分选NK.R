...
diff<-read.csv(file="C:/Users/a/Desktop/as分选课题/分选课题/1/CD4_res.csv")
...
display_number = c(11, 11, 11)#这三个数字分别代表选取的BP、CC、MF的通路条数，这个自己设置就行了
...
ggplot(data=go_enrich_df, aes(x=type_order,y=GeneNumber, fill=type)) + #横纵轴取值
  ...
  xlab("GO term") + 
  ylab("Gene_Number") + 
  labs(title = "The Most Enriched GO Terms of CD4")+
  theme_bw()
